...
1. Every section writer runs a compact embedded preflight for fabricated facts/citations, unresolved markers, causal overreach, and locale mismatch. This preflight uses the shared writing contract and does not load the complete `kiem-van-phong` skill or all of its references.
...
No legacy alias skill directories are shipped. Claude loads skill descriptions for routing and does not provide a reliable “explicit-only alias” contract, so alias directories would consume context and create duplicate triggers. Each canonical skill description and the migration guide may list relevant legacy names as migration keywords, for example “replaces `dieu-phoi-luan-van` and `viet-phan-vn`,” but only the canonical directory is callable. Direct invocation of a removed legacy `$skill` name is an intentional major-version break and receives a documented lookup path through `co-van`.
...
4. **Thesis/dissertation, document execution, and revision:** thesis profiles, full document assembly, semantic restructuring, Word backends, rendered validation, examination/peer review, response and commitment tracking. This slice has a named HMU deliverable: obtain and encode the current official thesis/dissertation presentation rules of Hanoi Medical University, with source snapshot, rule-level profile fields, `last_verified`, license/redistribution note, golden DOCX fixture, and structure/format tests. The document-profile workstream owns implementation; the plugin maintainer owns source authorization and acceptance. The due date is 2026-09-30 and in all cases before slice-4 acceptance. If an authoritative source is unavailable, slice 4 cannot claim HMU support; the maintainer must either resolve the source or formally remove HMU from the release scope rather than leave it indefinitely `PENDING LOCAL SOURCE`.
...
- No alias skill directories are permitted. Legacy names may appear only as migration keywords in canonical descriptions and documentation.
...
13. no legacy alias directory ships and migration keywords do not create overlapping canonical triggers;
...
- Portable core plus a Claude Chat/Cowork package first; Codex and ChatGPT Chat/Work adapters as separately accepted later deliverables.
- Twenty-four canonical skill directories with legacy migration keywords and no alias directories.
...
- Migration guide, third-party notices, changelog, rollback package, and final ZIP installer.
